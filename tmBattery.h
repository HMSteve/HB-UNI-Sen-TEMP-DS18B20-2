...
//
// 2025-09-07 HHSteve (Creative Commons)
// tmBatteryResDivHMSensor class added, as HMSensor board connects AREF and VCC
//---------------------------------------------------------
...
//---------------------------------------------------------
template <uint8_t SENSPIN, uint8_t ACTIVATIONPIN, uint16_t FACTOR, uint16_t VREF> class tmBatteryResDivExtVref : public tmBattery {
    // FACTOR: Rges/Rlow*1000, z.B. Teiler 470k/100k, Faktor 570k/100k*1000 = 5700
    // for HMSensor board: VREF vonnected to VCC, normally 3000 mV

public:
    tmBatteryResDivExtVref()
        : tmBattery()
    {
    }
    virtual ~tmBatteryResDivExtVref() {}
...
private:
    virtual uint16_t voltage()
    {
        // setup reference and input each time, just in case other SW modules have changed this
        ADMUX &= ~(ADCMUX_REFMASK | ADCMUX_ADCMASK);
        ADMUX |= ADCMUX_REF_AREF;                 // select Aref as reference, HMSensor connects the pin to AVcc!!!
        ADMUX |= ((SENSPIN - PIN_A0) & 0x0F);    // select SensePin as input
        ...
        if (adc) {
            // read SensePin against Bandgap:
            // ADC = Vin * 1024 / Vref -> Vin = Vref * ADC / 1024 (* resistor factor)
            uint32_t vbat = 1UL * VREF * adc * FACTOR / 1024 / 1000;
            DPRINT(F("tmBatteryResDivExtVref Voltage: "));
            DDECLN(vbat);
            return (uint16_t)vbat;
        } else {
            DPRINT(F("tmBatteryResDivExtVref Error"));
            return 0;
        }
    }
};
